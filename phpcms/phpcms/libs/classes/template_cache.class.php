		...
		if (in_array($op,$tools)) {
			switch ($op) {
				...
				case 'get':
						...
						if (isset($page)) {
							...
							$str .= '$r = $get_db->sql_query("'.$sql.'");$s = $get_db->fetch_next();$member_pages=member_pages($s[\'count\'], $page, $pagesize, $urlrule);';
						}
			...
			if (module_exists($op) && file_exists(PC_PATH.DIRECTORY_SEPARATOR.'modules'.DIRECTORY_SEPARATOR.$op.DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR.$op.'_tag.class.php')) {
				...
				if (isset($page)) {
					...
					$str .= '$member_pages = member_pages($'.$op.'_total, $page, $pagesize, $urlrule);';
				}
